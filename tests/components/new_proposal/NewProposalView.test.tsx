...
describe('NewProposalView', () => {
  test('renders sidebar navigation correctly', () => {
    ...
    expect(
      screen.getByText(/Refer to the documentation for detailed instructions on scheduling operations/i)
    ).toBeInTheDocument();
  ...
  test('renders Step 1 content correctly', () => {
    ...
    expect(
      screen.getByText(/Enter the address of the contract you wish to interact with and fetch its ABI/i)
    ).toBeInTheDocument();
    ...
    expect(screen.getByRole('button', { name: /Fetch ABI/i })).toBeInTheDocument();
  });
  ...
  test('shows success message after fetching ABI', () => {
    ...
    expect(screen.queryByText(/ABI fetched successfully!/i)).not.toBeInTheDocument();
  ...
  test('allows navigation to Step 2', () => {
    ...
    expect(screen.getByText(/Step 2: Configure Function Call/i)).toBeInTheDocument();
    expect(
      screen.getByText(/Select a function from the ABI and provide the required arguments/i)
    ).toBeInTheDocument();
  });
  ...
  test('allows input in contract address field', () => {
    ...
    const input = screen.getByLabelText(/Target Contract Address/i) as HTMLInputElement;
  ...
  test('allows input in function parameter fields', () => {
    ...
    const amountInput = screen.getByLabelText(/amount \(uint256\)/i) as HTMLInputElement;
  ...
  test('allows function selection from dropdown', () => {
    ...
    fireEvent.change(select, { target: { value: 'setOwner(address newOwner)' } });
  ...
  test('Back button navigates to previous step', () => {
    ...
    expect(screen.getByText(/Step 1: Select Target Contract/i)).toBeInTheDocument();
    expect(screen.queryByText(/Step 2: Configure Function Call/i)).not.toBeInTheDocument();
  });
