...
describe('DecoderView', () => {
  test('renders decoder elements correctly', () => {
    ...
    expect(screen.getByRole('heading', { name: /Calldata Decoder/i })).toBeInTheDocument();
    expect(
      screen.getByText(/Decode arbitrary calldata or transaction hashes from the Rootstock network/i)
    ).toBeInTheDocument();
    ...
    expect(screen.getByLabelText(/Contract Address \(Optional\)/i)).toBeInTheDocument();
    expect(screen.getByLabelText(/Contract ABI \(JSON, Optional\)/i)).toBeInTheDocument();
    ...
    expect(screen.getByRole('heading', { name: /Decoded Function/i })).toBeInTheDocument();
  ...
  test('renders input fields with correct placeholders', () => {
    ...
    expect(screen.getByPlaceholderText(/Paste contract ABI JSON here\.\.\./i)).toBeInTheDocument();
  });
  ...
  test('renders example decoded output', () => {
    ...
    expect(screen.getByText(/0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D/i)).toBeInTheDocument();
